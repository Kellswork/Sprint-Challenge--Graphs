...
roomGraph = {0: [(3, 5), {'n': 1, 's': 5, 'e': 3, 'w': 7}], 1: [(3, 6), {'s': 0, 'n': 2}], 2: [(3, 7), {'s': 1}],
             3: [(4, 5), {'w': 0, 'e': 4}], 4: [(5, 5), {'w': 3}], 5: [(3, 4), {'n': 0, 's': 6}], 6: [(3, 3), {'n': 5}],
...
# FILL THIS IN
traversalPath = ['n', 's']
# create a set for rooms visited
visited_room = set()
# add the first room to visited_room
visited_room.add(0)

# travel using dft
print(roomGraph[player.currentRoom.id][1].items())


def all_rooms_visited(room_id):
    for room in roomGraph[room_id][1].values():
...
def tryTracingBack():  # it's just a bfs
    ...
    while q.size() > 0:
        ...
        for searchedRoom in roomGraph[room][1].values():
            ...
            new_path = paths[room][:]
            # print('path-room', new_path)
            new_path.append(searchedRoom)

            # print('searched-room', searchedRoom)
            paths[searchedRoom] = new_path
            # print('path-searched-room', paths[searchedRoom])

            if not all_rooms_visited(searchedRoom):
                room_path = paths[searchedRoom]
                directions = []
                for i in range(len(room_path) - 1):
                    directions.append(room_directions(room_path[i], room_path[i + 1]))
                    visited_room.add(room_path[i + 1])
                return directions, room_path[len(room_path) - 1]
